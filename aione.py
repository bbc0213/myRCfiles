    ...
    if(request.method == 'OPTIONS'):
        ...
        if(realjson.__contains__('receiveNo')==True and 
            ...
            if os.path.exists('./GPUBUSY'):
                return jsonify({"resCode":409 , "rspMsg":"GPU正在忙碌中"})  
            else:
                if(f == 0):
                            ...
                            if(os.path.isdir(exitdir)):
                                ...
                                for f in files:
                                    ...
                                    if isdir(fullpath):
                                        ...
                                        subprocess.call("CUDA_VISIBLE_DEVICES="" python3 ./integration/slim/inference_three.py -abs_path /app/AIONE/integration/slim/ -s1_in " + "./exec/" + traceId + "/" + p8guid +"/" , shell = True)
                                        
                                        subprocess.call("./integration/ctbc_ocr " + "exec/" + traceId + "/" + p8guid , shell=True)
                    ...
                    r = requests.post('http://0.0.0.0:6080/services/process_management')
                    return 'ok'
                else:
                    return jsonify({"resCode":200 , "rspMsg":"送出成功,系統已收案"})
        else:
            r = requests.post('http://0.0.0.0:6080/services/process_management')
            os.makedirs('./JSONWRONG')
            return jsonify({"resCode":200 , "rspMsg":"json格式錯誤"})
...
@app.route('/services/process_management',methods=["POST"])
def process_management():
    ...
    if os.path.exists('./JSONWRONG'):
        TraceId = 'wrongjson'
        f = zipfile.ZipFile(TraceId + '.zip','w')
            
        f.write(os.path.join('./wrongjson.vml'))

        f.close

        with open(TraceId + '.zip','rb') as f:
            file_base64 = f.read()
        ...
        f = zipfile.ZipFile('./exec/' + TraceId + '.zip','w')
            
        f.write(os.path.join('./filenetwrong.vml'))

        f.close

        with open('./exec/' + TraceId + '.zip','rb') as f:
            file_base64 = f.read()
        ...
        if(receiveNO == '00000000'):
            ...
            f = zipfile.ZipFile('./exec/' + vmlTraceId + '.zip','w')
            ...
            with open('./exec/' + vmlTraceId + '.zip','rb') as f:
                file_base64 = f.read()           
                base64_data = base64.b64encode(file_base64)

        else:

            f = zipfile.ZipFile('./exec/' + TraceId + '.zip','w',zipfile.ZIP_DEFLATED) 
    ...
    #print('json :',d)
    #print('url: http://192.168.1.101:31884/CTBC/services/RS/aione')
